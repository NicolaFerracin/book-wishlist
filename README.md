...
- **Price checking** — scrapes BookFinder (aggregates AbeBooks, Amazon, Biblio, ThriftBooks, Alibris, etc.) using Playwright
...
BookFinder is an aggregator that searches AbeBooks, Amazon, Biblio, ThriftBooks, Alibris, and many more stores in one go. Prices include shipping to your selected destination. The scraper uses Playwright (headless Chromium) and runs server-side so you can close the browser tab while it works.
